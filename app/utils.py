...
def light_assert(exp, message, cont = True):
    """
...
def check_json(result, expectation, path = "$"):
    no_error = True
    orig_path = path
    for key in expectation:
        ...
        elif not light_assert(result.has_key(key), "The result does not have the path : {}".format(path)):
            no_error = False
            continue
        ...
        if isinstance(exp, dict):
            check_json(res, exp, path)

        elif isinstance(exp, list):
            pattern = ""
            if len(exp) > 0 :
                no_error = light_assert(re.match('#.*#', str(exp[0])),
            ...
            if len(exp) == 0:
                no_error = light_assert(len(res) == 0,
            ...
            # Check that we have the same number of entries
                ...
                no_error = light_assert(len(res) == int(re.findall('#=([0-9]+)#', exp[0])[0]),
            ...
            if exp.startswith("#r#"):
                exp = exp.split('#r#')[-1]
                reg = re.compile(exp)
                no_error = light_assert(reg.match(res),
                             ('The result "{}" does not match the regex "{}"'
                              '\n* PATH : {}').format(res, exp, path))
            else:
                no_error = light_assert(res == exp, 
